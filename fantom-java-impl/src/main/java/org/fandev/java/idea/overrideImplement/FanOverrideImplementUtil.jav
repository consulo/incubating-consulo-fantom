...
public class FanOverrideImplementUtil
{
	...
	public static void invokeOverrideImplement(final Project project, final Editor editor, final PsiFile file, boolean isImplement)
	{
		...
		final MemberChooser<PsiMethodMember> chooser = new MemberChooser<PsiMethodMember>(classMembers.toArray(new PsiMethodMember[0]), false, true,
				project);
	...
	public static Collection<CandidateInfo> getMethodsToOverrideImplement(PsiClass clazz, boolean flag)
	{
		...
		for(final PsiMethod method : allMethods)
		{
			...
			{
				final MethodSignature signature = MethodSignatureUtil.createMethodSignature(method.getName(), method.getParameterList(),
	...
	private static FanMethod createOverrideImplementMethodSignature(final Project project, final PsiMethod method, final PsiSubstitutor substitutor,
	...
	private static void setupOverridingMethodBody(final Project project, final PsiMethod method, final FanMethod resultMethod,
			final FileTemplate template, final PsiSubstitutor substitutor, final Editor editor)
	{
		final PsiType returnType = substitutor.substitute(method.getReturnType());
