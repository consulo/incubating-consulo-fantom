...
public class FanPodCreatePanel extends JComponent
{
	...
	public FanPodCreatePanel(final FanModuleBuilder builder)
	{
		...
		final BrowseFilesListener srcDirsBrowseListener = new BrowseFilesListener(srcDirs, FanBundle.message("src.dirs.path"),
				FanBundle.message("src.dirs.path.description"), srcDirsChooser)
		{
			@Override
			public void actionPerformed(final ActionEvent e)
			{
				final VirtualFile fileToSelect = getFileToSelect();
				final VirtualFile[] files = (fileToSelect != null) ? FileChooser.chooseFiles(srcDirs, srcDirsChooser,
						fileToSelect) : FileChooser.chooseFiles(srcDirs, srcDirsChooser);
		...
		final BrowseFilesListener resDirsBrowseListener = new BrowseFilesListener(resDirs, FanBundle.message("res.dirs.path"),
				FanBundle.message("res.dirs.path.description"), resDirsChooser)
		{
			@Override
			public void actionPerformed(final ActionEvent e)
			{
				final VirtualFile fileToSelect = getFileToSelect();
				final VirtualFile[] files = (fileToSelect != null) ? FileChooser.chooseFiles(resDirs, resDirsChooser,
						fileToSelect) : FileChooser.chooseFiles(resDirs, resDirsChooser);
				final StringBuilder sb = new StringBuilder();
				for(int i = 0; i < files.length; i++)
				{
					if(i != 0)
		...
		final BrowseFilesListener outDirBrowseListener = new BrowseFilesListener(outDir, FanBundle.message("out.dir.path"),
				FanBundle.message("out.dir.path.description"), outDirChooser)
		{
			@Override
			public void actionPerformed(final ActionEvent e)
			{
				final VirtualFile fileToSelect = getFileToSelect();
				final VirtualFile[] files = (fileToSelect != null) ? FileChooser.chooseFiles(outDir, outDirChooser,
						fileToSelect) : FileChooser.chooseFiles(outDir, outDirChooser);
				final StringBuilder sb = new StringBuilder();
				for(int i = 0; i < files.length; i++)
				{
					if(i != 0)
		...
		if(moduleFile != null)
		{
			...
			dependenciesChooser.setRoot(VirtualFileUtil.refreshAndFindFileByLocalPath(VirtualFileUtil.buildUrl(builder.getSdk().getHomePath(), "lib/fan")));
			dependenciesChooser.addRoot(v);
		}
		final BrowseFilesListener dependenciesBrowseListener = new BrowseFilesListener(dependencies, FanBundle.message("out.dir.path"),
				FanBundle.message("out.dir.path.description"), dependenciesChooser)
		{
			@Override
			public void actionPerformed(final ActionEvent e)
			{
				final VirtualFile fileToSelect = getFileToSelect();
				final VirtualFile[] files = (fileToSelect != null) ? FileChooser.chooseFiles(dependencies, dependenciesChooser,
						fileToSelect) : FileChooser.chooseFiles(dependencies, dependenciesChooser);
				final StringBuilder sb = new StringBuilder();
				for(int i = 0; i < files.length; i++)
				{
		...
		final FieldPanel dependenciesFieldPanel = new FieldPanel(dependencies, FanBundle.message("pod.dependencies"), null, dependenciesBrowseListener,
				null);
		...
		final FanFieldPanel indexesFieldPanel = new FanFieldPanel(indexes, FanBundle.message("pod.indexes"), FanBundle.message("pod.indexes"),
				indexesListener, null);
		final FanFieldPanel metasFieldPanel = new FanFieldPanel(metas, FanBundle.message("pod.metas"), FanBundle.message("pod.metas"), metasListener,
				null);
