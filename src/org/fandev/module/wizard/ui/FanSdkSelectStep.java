...
public class FanSdkSelectStep extends ModuleWizardStep
{
	...
	public boolean validate()
	{
		...
		if(jdk == null)
		{
			int result = Messages.showYesNoDialog(FanBundle.message("sdk.error.no.sdk.prompt.messge.confirm.without.sdk"),
					FanBundle.message("sdk.select.prompt.title"), Messages.getWarningIcon());
			return result == DialogWrapper.OK_EXIT_CODE;
		}
		if(!(jdk.getSdkType() instanceof FanSdkType))
		{
			Messages.showErrorDialog(FanBundle.message("sdk.error.prompt.message.sdk.not.valid"), FanBundle.message("sdk.select.prompt.title"));
			return false;
		}
