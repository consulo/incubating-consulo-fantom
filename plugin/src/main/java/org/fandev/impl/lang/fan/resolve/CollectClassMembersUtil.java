package org.fandev.impl.lang.fan.resolve;
...
/*	private static final Key<CachedValue<Pair<Map<String, CandidateInfo>, Map<String, List<CandidateInfo>>>>> CACHED_MEMBERS = Key.create
	...
	private static final Key<CachedValue<Pair<Map<String, CandidateInfo>, Map<String, List<CandidateInfo>>>>> CACHED_MEMBERS_INCLUDING_SYNTHETIC = Key
			.create("CACHED_MEMBERS_INCLUDING_SYNTHETIC");
	...
	{
		return aClass.getManager().getCachedValuesManager().createCachedValue(new CachedValueProvider<Pair<Map<String, CandidateInfo>, Map<String,
				List<CandidateInfo>>>>()
		{
			public Result<Pair<Map<String, CandidateInfo>, Map<String, List<CandidateInfo>>>> compute()
			{
				...
				return new Result<Pair<Map<String, CandidateInfo>, Map<String, List<CandidateInfo>>>>(new Pair<Map<String, CandidateInfo>, Map<String,
						List<CandidateInfo>>>(allFields, allMethods), PsiModificationTracker.OUT_OF_CODE_BLOCK_MODIFICATION_COUNT);
			}
		}, false);
	}

	private static void processClass(final PsiClass aClass, final Map<String, CandidateInfo> allFields, final Map<String,
			List<CandidateInfo>> allMethods, final Set<PsiClass> visitedClasses, final PsiSubstitutor substitutor, final boolean includeSynthetic)
	{
		...
		for(final PsiMethod method : includeSynthetic || !(aClass instanceof FanTypeDefinition) ? aClass.getMethods() : ((FanTypeDefinition) aClass)
				.getFanMethods())
		{
